...
Eu, \emph{Hugo de Barros Araujo}, autor da monografia \emph{SISTEMAS MULTIAGENTE E APRENDIZADO POR REFORÇO APLICADOS A META-HEURÍSTICAS PARA OTIMIZAÇÃO DO PROBLEMA DE ROTEAMENTO DE VEÍCULOS}, subscrevo para os devidos fins, as seguintes informações:\\
...
\paragraph{}A originalidade da proposta reside na encapsulação desses algoritmos como agentes especialistas dentro do \textit{framework} Mesa, onde a cooperação é estabelecida através de uma base de soluções de elite e da estratégia de \textit{Path Relinking}, que explora trajetórias entre soluções de alta qualidade para evitar ótimos locais. Explorou-se, ainda, a aplicação do algoritmo \textit{Q-Learning} como uma hiper-heurística capaz de aprender, de forma autônoma, qual especialista acionar em diferentes estados do espaço de busca. Os resultados demonstram que a inteligência coletiva do sistema multiagente (MAS) supera o desempenho das meta-heurísticas isoladas. Ademais, constatou-se que a adaptação dinâmica via RL apresenta uma eficácia estatisticamente equivalente à do MAS puro, validando a viabilidade do aprendizado por reforço na orquestração de agentes e proporcionando soluções otimizadas em tempo computacional viável.
...
\paragraph{}The originality of this approach lies in encapsulating these algorithms as specialized agents within the Mesa framework, where cooperation is established through a shared elite solution pool and the Path Relinking strategy, which explores trajectories between high-quality solutions to circumvent local optima. Furthermore, the Q-Learning algorithm was explored as a hyper-heuristic capable of autonomously learning which specialist to activate across different states of the search space. The results demonstrate that the collective intelligence of the multi-agent system (MAS) outperforms the isolated meta-heuristics. Additionally, it was found that the dynamic adaptation via RL achieves an efficacy statistically equivalent to that of the pure MAS, validating the feasibility of reinforcement learning in agent orchestration and providing optimized solutions within viable computational time.
...
\paragraph{}MAS -- \textit{Multi-Agent System} (sistema multiagente)
\paragraph{}MAS-RL -- \textit{Multi-Agent System with Reinforcement Learning} (sistema multiagente com aprendizado por reforço)
...
\paragraph{}MILP -- \textit{Mixed Integer Linear Programming} (programação linear inteira mista)
\paragraph{}BKS -- \textit{Best Known Solution} (melhor solução conhecida)
