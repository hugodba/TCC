...
\paragraph{} Um componente crítico nesta etapa foi a função custo compartilhada, desenvolvida de forma modular para garantir que a avaliação das rotas fosse consistente entre os diferentes algoritmos testados. Ela é definida de forma que a qualidade de uma solução $s$ seja avaliada por ordem lexicográfica do par $(K(s), D(s))$, onde $K(s)$ é o número de veículos e $D(s)$ a distância total. Uma solução $s_1$ é preferível a $s_2$ se $K(s_1) < K(s_2)$, ou se $K(s_1) = K(s_2)$ e $D(s_1) < D(s_2)$, reproduzindo a hierarquia de objetivos de Solomon. 

\paragraph{} Para validar a implementação, utilizou-se como referência a instância RC201 de Solomon (cujos resultados e procedimentos de referência encontram-se descritos, respectivamente, em \cite{Sintef100, Mester05}). A figura abaixo apresenta a distribuição espacial dos clientes nesta instância.
...
\paragraph{} Um aspecto metodológico relevante na modelagem dos algoritmos é a adoção da representação de solução por meio de uma codificação de \textit{giant tour} com divisão gulosa (\textit{greedy split}). Nessa abordagem, o espaço de busca não segrega soluções entre viáveis e inviáveis a priori; a inserção de retornos ao depósito central é acionada dinamicamente pela impossibilidade operacional de atender ao próximo cliente da sequência, seja por violação da restrição de capacidade de carga ou pelo fechamento da janela temporal. Embora essa estratégia reduza significativamente a complexidade computacional da exploração de vizinhanças, assume-se como limitação inerente a tendência de superestimar o número total de veículos ($K$), operando com frotas conservadoras em prol da velocidade de convergência e simplificação estrutural das rotas.
