...
\paragraph{} Dado que as meta-heurísticas podem continuar executando por tempo indefinido, os seus hiperparâmetros foram escolhidos balizados pela performance e estagnação. Ou seja, ao longo de testes preliminares, buscou-se um resultado ótimo e o encerramento da execução o mais rápido possível após atingi-lo. Os hiperparâmetros globais de controle do MAS e do RL também buscaram resultados ótimos, porém sem a preocupação de limitar o tempo de execução quando estagnado.

\paragraph{} Para fins de controle experimental e reprodutibilidade, implementou-se o uso de \textit{seeds} fixas nos geradores de números aleatórios. Cumpre ressalvar, contudo, que tal procedimento assegura o determinismo estrito apenas nas meta-heurísticas monoprocesso isoladas. No âmbito do Sistema Multiagente, em virtude do paralelismo via \texttt{ProcessPoolExecutor} e da coleta assíncrona de soluções, a ordem exata de inserção dos indivíduos no \textit{ElitePool} torna-se dependente do escalonador de processos do sistema operacional. Consequentemente, para mitigar essa variabilidade assíncrona e garantir a validade estatística perante a natureza estocástica do ambiente cooperativo, cada algoritmo foi executado 10 vezes independentes para cada instância, adotando-se as médias e desvios padrão dessas rodadas como base comparativa nas análises computacionais.
...
\paragraph{} Cabe registrar que, em termos absolutos, os valores obtidos para o tamanho da frota ($K$) ao longo de todos os experimentos permanecem distantes das melhores soluções conhecidas. Essa disparidade expressiva não decorre de uma ineficácia na capacidade de busca das meta-heurísticas em si, mas reflete uma limitação estrutural da modelagem adotada. Conforme fundamentado na \autoref{subsec:giant_tour}, a representação de soluções baseada em \textit{giant tour} com fatiamento guloso simples (\textit{greedy split}) introduz uma superestimação sistemática no número de rotas ao realizar partições locais sem uma avaliação global de partição ótima. Esse fenômeno manifesta-se de forma acentuada nas instâncias de Série 2, onde a elevada capacidade dos veículos e a flexibilidade das janelas temporais teoricamente permitiriam frotas extremamente reduzidas. Em instâncias agrupadas como \texttt{c201.txt} e \texttt{c202.txt}, cujo ótimo físico é $K^* = 3$, a heurística de divisão simples gera soluções que demandam de $3$ a $4$ vezes mais veículos que a BKS. Sob essa perspectiva, a comparação aqui conduzida permanece de caráter estritamente relativo entre as arquiteturas implementadas sob condições idênticas de representação, e não frente ao estado da arte de solvers exatos de VRPTW.
...
\paragraph{} Note que as linhas referentes a \textit{MAS} e \textit{MAS-RL} começam em um tempo maior, devido à arquitetura de implementação que recolhe as soluções parciais apenas após o término de cada \textit{step}, diferentemente das meta-heurísticas puras que atualizam a solução a cada iteração.
...
\paragraph{} Um teste de hipóteses foi conduzido com o objetivo de determinar se as diferenças de desempenho observadas entre os métodos propostos possuem significância estatística. Uma vez que os dados de desempenho computacional raramente seguem uma distribuição normal em otimização estocástica, e considerando que as medições foram realizadas de forma pareada nas mesmas instâncias de Solomon, abordagens não-paramétricas foram aplicadas. Registra-se, como delimitação formal de escopo, que os testes globais de Friedman e pós-hoc de Nemenyi foram executados estritamente sobre a métrica de tamanho da frota ($K$). A omissão da distância total percorrida ($D$) desta validação estatística específica justifica-se pela natureza estritamente hierárquica da função objetivo do VRPTW, na qual a minimização do número de veículos constitui o objetivo primário e de maior impacto econômico na operação logística. Desse modo, $K$ consolida-se como a variável crítica e isolada para aferir a eficácia macro das estruturas cooperativas propostas.
...
\begin{itemize}
    \item \textbf{Hipótese Nula ($H_0$):} As distribuições dos postos (\textit{ranks}) de todos os métodos avaliados são estatisticamente equivalentes, assumindo que as amostras provêm da mesma população contínua e não possuem diferenças de desempenho.
    \item \textbf{Hipótese Alternativa ($H_1$):} Existe uma diferença estatisticamente significativa na distribuição dos postos de, pelo menos, um dos algoritmos testados em relação aos demais.
\end{itemize}
...
\paragraph{} Para ilustrar estas conclusões de forma visual, a Figura \ref{fig:cd_diagram} apresenta o Diagrama de Diferença Crítica (\textit{Critical Difference Diagram}). Para garantir que a interpretação geométrica do gráfico seja autocontida, registra-se que a tolerância da Diferença Crítica ($CD$) foi dimensionada considerando $k = 5$ algoritmos avaliados em $N = 12$ instâncias independentes. Adotando-se o limite crítico da distribuição de Nemenyi para um nível de significância de $\alpha = 0.05$ ($q_{\alpha} = 2.728$), a métrica de corte estabelecida foi de $CD = 1.76$. De acordo com o procedimento do teste, algoritmos que não se encontram conectados por barras horizontais espessas possuem uma distância entre seus \textit{ranks} médios estritamente superior a $1.76$, atestando de maneira conclusiva e matemática a superioridade dos arranjos cooperativos.

\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.85\textwidth]{cd_diagram.png}
    \caption[\small{Diagrama de Diferença Crítica (Nemenyi) para a minimização de Veículos.}]{\label{fig:cd_diagram} \small{Diagrama de Diferença Crítica (Nemenyi) para a minimização de Veículos ($CD = 1.76$).}}
\end{figure}
